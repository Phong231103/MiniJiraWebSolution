...
namespace Web.Application.Auth.Commands.Login
{
    ...
    public class LoginCommandHandler : IRequestHandler<LoginCommand, Result<AuthResponse>>
    {
        ...
        public async Task<Result<AuthResponse>> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            var token = _token.GenerateToken(user.First());

            var refreshTokenstring = _token.GenerateRefreshToken();

            var refreshToken = RefreshToken.Create(user.First().Id, user.First().Devices.)
